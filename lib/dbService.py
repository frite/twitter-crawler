...
def store_user(userObject):
    #  Store user in db
    creationDay = userObject.created_at
    dbUser = persons(user_id=userObject.id, screenname=userObject.screen_name, name=userObject.name,
                     description=userObject.description, listed_count=userObject.listed_count,
                     friends_count=userObject.friends_count, statuses_count=userObject.statuses_count,
                     followers_count=userObject.followers_count, favorites_count=userObject.favourites_count,
                     url=userObject.url, geo_enable=userObject.geo_enabled, location=userObject.location,
                     language=userObject.lang, protected_profile=userObject.protected, createdAt=creationDay)
    insert(dbUser, 'An exception flew by while storing the user!')


def store_tweets(tweetObjects, userObject):
    #  Store tweet in db
    ...
    for tweet in tweetObjects:
        if hasattr(tweet, 'retweeted_status'):
            ...
            dbTweet = tweets(tweet_id=tweet.retweeted_status.id, user_id=tweet.retweeted_status.user.id,
                             status=tweet.retweeted_status.text, countFavorites=tweet.favorite_count,
                             countRTs=tweet.retweet_count, time=tweet.retweeted_status.created_at)
            insert(dbTweet, 'An exception flew by while storing the tweet (1)!')
            store_tweet_entities(tweet)
            store_retweet(tweet.retweeted_status.id, userObject.id, tweet.created_at)
        else:
            creationDay = tweet.created_at
            dbTweet = tweets(tweet_id=tweet.id, user_id=tweet.user.id, status=tweet.text,
                             countFavorites=tweet.favorite_count, countRTs=tweet.retweet_count, time=tweet.created_at)
...
def store_hashtags(tweet):
    ...
    if tweet.entities['hashtags'] is not None:
        for hashtag in tweet.entities['hashtags']:
            ...
            store_tweet_hashes(tweet.id, hashtag['text'])


def store_urls(tweet):
    # Store urls in db
    if tweet.entities['urls'] is not None:
        for url in tweet.entities['urls']:
            dbURL = urls(category_id=match_category(url['expanded_url']), expanded_url=url['expanded_url'],
                         content=None)
            insert(dbURL, 'An exception flew by while storing the url!')
            store_tweet_urls(tweet.id, url['expanded_url'])


def store_followers(followersObject, user):
...
def store_friends(friendsObject, user):
    #  Store friends in db
...
def store_favorites(userObject, favorites):
    # Store favorites, requires authentication
    store_tweets(favorites)
...
def store_tweet_hashes(tweetID, szhashtag):
    # Relationship of tweets-hashtags
    db_connect()
...
def store_tweet_urls(tweetID, url):
    #  Relationship of tweets-urls
    db_connect()
    try:
        ...
        print_exception("Url not found")


def init():
    #  First call, it'll create tables, or it will raise problems
...
def print_exception(string, e=None):
    oTextColor.warning("\t[WARNING*]\t" + string)
    if e is not None:
        oTextColor.warning(str(e))


def match_category(url):
    #  Matches a category
    if url.find("facebook") != -1:
        return 1
    elif url.find("youtube") != -1 or url.find("youtu.be") != -1:
        return 3
    elif url.find("blogspot") != -1 or url.find("blogger") != -1 or url.find("tumblr") != -1 or url.find(
            "wordpress") != -1:
        return 2
    else:
        return 4
